    ...
    def solve(self):
        ...
        if (self.all_grains_percentage()):
            if ('total_grain' in self.properties):
                ...
                raise MissingParam(
                    "Cannot solve mash as have only grain ratios")
        ...
        if (self.has_properties('hc_initial', 't_initial')):
            ...
            hc_grain = Quantity(mass_grain.to(
                'lb') * shc_grain.to('Btu/lb/F'), 'Btu/F')
        ...
        hc_water = Quantity(mass_water.to('lb') *
                            shc_water.to('Btu/lb/F'), 'Btu/F')
        hc_total = hc_water + hc_initial
        t_strike = Quantity(((hc_total.to('Btu/F') * t_mash -
                              hc_initial.to('Btu/F') * t_initial.to('F')) /
                             hc_water.to('Btu/F')), 'F')
