...
class Quantity:
    ...
    def to(self, new_unit):
        ...
        e.g. lbs = weight.to('lb')
             ...
             f = temp.to('F')
        """
        # Make sure current and new units are canonical
        self.unit = self.canonical_unit(self.unit)
    ...
    def find_conversion(from_unit, to_unit):
        ...
        if (not (from_unit in Quantity.ALL_CONV)):
            raise ConversionError(
                'unknown original unit in conversion requested from %s to %s' % (from_unit, to_unit))
        if (not (to_unit in Quantity.ALL_CONV)):
            raise ConversionError(
                'unknown destination unit in conversion requested from %s to %s' % (from_unit, to_unit))
        if (not (to_unit in Quantity.ALL_CONV[from_unit])):
            raise ConversionError(
                'unknown conversion requested from %s to %s' % (from_unit, to_unit))
    ...
    def _build_ALL_CONV():
        ...
        #
        # Build local self.conv with data from Quantity.CONVERSIONS and
        # inverses
        Quantity.ALL_CONV = {}
        for f in Quantity.CONVERSIONS:
            ...
            for t in Quantity.CONVERSIONS[f]:
                ...
                if (not (f in Quantity.ALL_CONV[t])):
                    Quantity.ALL_CONV[t][f] = 1.0 / \
                        Quantity.ALL_CONV[f][t]  # inverse
            ...
            for m in list(Quantity.ALL_CONV[s].keys()):
                for e in Quantity.ALL_CONV[m]:
                    if (not (e in Quantity.ALL_CONV[s])):
                        Quantity.ALL_CONV[s][e] = Quantity.ALL_CONV[
                            s][m] * Quantity.ALL_CONV[m][e]
    ...
    c_data = [-0.0005, 0.0, 0.001, 0.002, 0.003,
    ...
    def hydrometer_correction(self, temp='60F'):
        ...
        if (tval < self.t_data[0] or tval > self.t_data[-1]):
            raise NameError(
                'Temperature out of range for hydrometer, must be within 50 - 118F')
